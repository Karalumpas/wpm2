...
const SAMPLE_PRODUCTS = [
  ...
  },
  {
    ...
    description: 'Cotton blend basic tee',
    basePrice: 19.99,
    type: 'variable',
  },
  {
...
async function seedBrands() {
  ...
  await db.insert(brands).values(brandInserts).onConflictDoNothing();
...
async function seedProducts() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const suffix =
      i > SAMPLE_PRODUCTS.length - 1
        ? ` ${Math.floor(i / SAMPLE_PRODUCTS.length) + 1}`
        : '';
    ...
    const productBrandIds = randomChoices(
      allBrands,
      Math.random() > 0.7 ? 2 : 1
    );
    ...
    const productCategoryIds = randomChoices(
      allCategories,
    ...
    if (product.type === 'variable') {
      ...
      for (let v = 0; v < variantCount; v++) {
        ...
        if (sample.name.includes('iPhone') || sample.name.includes('Samsung')) {
          ...
          sample.name.includes('Shoes') ||
          sample.name.includes('T-Shirt') ||
          sample.name.includes('Jeans')
        ) {
          attributes.color = randomChoice(VARIANT_COLORS);
  ...
  console.log(
    `✓ Seeded ${productCount} products with ${totalVariantCount} variants`
  );
}

async function updateProductTimestamps() {
  console.log(
    'Updating product timestamps for realistic pagination testing...'
  );
