...
import { db } from '@/db';
import { products, productVariants, productCategories, productBrands } from '@/db/schema';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const deletedVariants = await db.delete(productVariants).returning({ id: productVariants.id });
    ...
    const totalAffected = deletedVariants.length + 
