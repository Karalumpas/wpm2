...
function ShopCard({ shop, onUpdate }: ShopCardProps) {
  ...
  const testConnection = async () => {
    try {
      ...
      addTestLog(`📊 Received response: ${response.status} ${response.statusText}`);
      setTestProgress(50);
      ...
      if (result.success) {
        ...
        if (result.details) {
          addTestLog(`📈 Details: WordPress=${result.details.reachable ? '✅' : '❌'}, WooCommerce=${result.details.auth ? '✅' : '❌'}, Products=${result.details.productsOk ? '✅' : '❌'}`);
      ...
      addTestLog(`💥 Unexpected error: ${err instanceof Error ? err.message : 'Unknown error'}`);
      setTestResult({
        success: false,
  ...
  const syncProducts = async () => {
    try {
      ...
      addSyncLog(`📊 Received response: ${response.status} ${response.statusText}`);
      setSyncProgress(30);
      ...
      if (result.success) {
        ...
        if (result.details) {
          addSyncLog(`📈 Summary:`);
          addSyncLog(`  📦 Products: ${result.details.productsCreated} created, ${result.details.productsUpdated} updated`);
          addSyncLog(`  📂 Categories: ${result.details.categoriesCreated} created, ${result.details.categoriesUpdated} updated`);
          addSyncLog(`  🎨 Variations: ${result.details.variationsCreated} created, ${result.details.variationsUpdated} updated`);
        ...
        addSyncLog(`❌ Synchronization failed: ${result.message || 'Unknown error'}`);
      ...
      addSyncLog(`💥 Unexpected error: ${err instanceof Error ? err.message : 'Unknown error'}`);
      setSyncResult({
        success: false,
              ...
              <div>
                ...
                {testResult.details && (
                  <ul className="mt-1 text-xs space-y-1">
                    ...
                    <li>Products API: {testResult.details.productsOk ? '✓' : '✗'}</li>
          ...
          <div className={`mt-3 p-3 rounded-md text-sm ${
            ...
            {syncResult.success ? (
              <div>
                ...
                {syncResult.details && (
                  <ul className="mt-1 text-xs space-y-1">
                    <li>Products: {syncResult.details.productsCreated + syncResult.details.productsUpdated} total</li>
                    <li>Categories: {syncResult.details.categoriesCreated + syncResult.details.categoriesUpdated} total</li>
                    <li>Variations: {syncResult.details.variationsCreated + syncResult.details.variationsUpdated} total</li>
                    {syncResult.details.errors.length > 0 && (
                      <li className="text-yellow-600">⚠️ {syncResult.details.errors.length} warnings</li>
                    )}
                  </ul>
                )}
              </div>
