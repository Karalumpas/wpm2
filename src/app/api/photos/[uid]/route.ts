...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error(
      `Error fetching photo ${await params.then((p) => p.uid)}:`,
      error
    );
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const photoWithUrls = {
      ...
      previewUrl: photoPrismClient.getPhotoThumbnailUrl(
        updatedPhoto.UID,
        'fit_720'
      ),
      fullUrl: photoPrismClient.getPhotoThumbnailUrl(
        updatedPhoto.UID,
        'fit_1920'
      ),
    ...
    console.error(
      `Error updating photo ${await params.then((p) => p.uid)}:`,
      error
    );
