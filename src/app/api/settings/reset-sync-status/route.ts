...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedVariants = await db.update(productVariants)
      .set({ lastSyncedAt: null })
      .returning({ id: productVariants.id });
    console.log(`🔸 Reset sync status for ${updatedVariants.length} product variants`);
    
    // Reset lastSyncedAt for categories
    const updatedCategories = await db.update(categories)
      .set({ lastSyncedAt: null })
      .returning({ id: categories.id });
    console.log(`🔸 Reset sync status for ${updatedCategories.length} categories`);
    
    const totalAffected = updatedProducts.length + updatedVariants.length + updatedCategories.length;
