...
async function getDashboardData() {
  ...
  const recentProducts = await db
    .select({ id: products.id, name: products.name, sku: products.sku, updatedAt: products.updatedAt })
    .from(products)
    .orderBy(desc(products.updatedAt))
    .limit(5);

  const shopsList = await db
    .select({ id: shops.id, name: shops.name, lastConnectionOk: shops.lastConnectionOk, status: shops.status })
    .from(shops)
...
const quickActions = [
  ...
  { name: 'Analytics', href: '/analytics', icon: BarChart3, description: 'View catalog analytics' },
] as const;
...
export default async function Home() {
  ...
  const statCards = [
    { name: 'Products', value: data.counts.products, icon: Package, href: '/products' },
    { name: 'Variants', value: data.counts.variants, icon: Package, href: '/products' },
    { name: 'Categories', value: data.counts.categories, icon: Tags, href: '/categories' },
    { name: 'Shops', value: data.counts.shops, icon: Wifi, href: '/connections' },
  ] as const;
    ...
    <div className="space-y-8">
      ...
      <div className="px-4 sm:px-6 lg:px-8">
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="ml-16 text-sm font-medium text-gray-700 truncate">{stat.name}</p>
                </div>
        ...
        <div className="mt-3 text-sm text-gray-700">
          <span className="inline-flex items-center mr-4"><CheckCircle2 className="h-4 w-4 text-emerald-600 mr-1" /> Published: {data.counts.published}</span>
          <span className="inline-flex items-center"><Clock className="h-4 w-4 text-gray-500 mr-1" /> Drafts: {data.counts.drafts}</span>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 xl:col-span-2">
          <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-900">Sync Activity</h3>
            <Link href="#" className="text-sm text-indigo-600 hover:text-indigo-700">Open Sync Panel</Link>
          </div>
          <div className="p-4">
            {data.jobs.length === 0 ? (
                ...
                {data.jobs.map((j) => (
                  <li key={j.id} className="py-3 flex items-center justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-700 truncate">{j.message || (j.status === 'running' ? 'Running…' : j.status)}</div>
                    </div>
                    <div>
                      {j.status === 'completed' ? (
                        <span className="px-2 py-1 text-xs rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Completed</span>
                      ) : j.status === 'failed' ? (
                        <span className="px-2 py-1 text-xs rounded-full bg-rose-50 text-rose-700 border border-rose-200">Failed</span>
                      ) : j.status === 'paused' ? (
                        <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-700 border border-gray-200">Paused</span>
                      ) : (
                        <span className="px-2 py-1 text-xs rounded-full bg-blue-50 text-blue-700 border border-blue-200">{j.status}</span>
                      )}
                    </div>
                  </li>
                ))}
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200">
          <div className="px-4 py-3 border-b border-gray-200">
            <h3 className="text-base font-semibold text-gray-900">Shops Health</h3>
          </div>
          <div className="p-4">
            {data.shops.length === 0 ? (
              ...
              <ul className="space-y-3">
                {data.shops.map((s) => (
                  <li key={s.id} className="flex items-center justify-between">
                    ...
                    {s.lastConnectionOk ? (
                      <span className="inline-flex items-center text-xs px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200"><CheckCircle2 className="h-3.5 w-3.5 mr-1" /> OK</span>
                    ) : (
                      <span className="inline-flex items-center text-xs px-2 py-1 rounded-full bg-amber-50 text-amber-800 border border-amber-200"><AlertTriangle className="h-3.5 w-3.5 mr-1" /> Check</span>
                    )}
                  </li>
                ))}
              </ul>
      ...
      <div className="px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow border border-gray-200">
          <div className="px-4 py-3 border-b border-gray-200">
            <h3 className="text-base font-semibold text-gray-900">Recently Updated Products</h3>
          </div>
          <div className="p-4">
              ...
              <ul className="divide-y divide-gray-200">
                {data.recentProducts.map((p) => (
                  <li key={p.id} className="py-3 flex items-center justify-between">
                    ...
                    <Link href={`/products/${p.id}`} className="text-xs text-indigo-600 hover:text-indigo-700">View</Link>
                  </li>
                ))}
              </ul>
            )}
