...
async function fetcher(url: string): Promise<JobsResponse> {
  ...
  console.debug(
    `🔄 [${timestamp}] Polling sync jobs: ${url.includes('jobId') ? 'specific job' : 'all jobs'}`
  );
  ...
  const activeJobs = jobs.filter(
    (job: SyncJob) => job.status === 'running' || job.status === 'queued'
  );
...
export function useSyncJobs(
  jobId?: string,
  uiContext?: { sidebarCollapsed?: boolean }
) {
