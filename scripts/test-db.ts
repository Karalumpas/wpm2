...
// Direct connection using postgres
...
async function testDatabaseConnection() {
  ...
  try {
    ...
    const result = await dbClient`SELECT NOW() as current_time, version() as version`;
    ...
    const dbCheck = await dbClient`SELECT current_database() as db_name`;
    ...
    const migrationTableExists = tables.some((table: any) => table.table_name === '__drizzle_migrations');
    if (migrationTableExists) {
      console.log('\n4. Checking migration status...');
      const migrations = await dbClient`
    ...
    if (existingTables.includes('users')) {
      ...
      const userCount = await dbClient`SELECT COUNT(*) as count FROM users`;
      console.log(`   Users: ${userCount[0].count} records`);
    }

    if (existingTables.includes('shops')) {
      const shopCount = await dbClient`SELECT COUNT(*) as count FROM shops`;
      console.log(`   Shops: ${shopCount[0].count} records`);
    }

    if (existingTables.includes('products')) {
      const productCount = await dbClient`SELECT COUNT(*) as count FROM products`;
