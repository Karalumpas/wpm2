...
export default function ProductWindow({ id }: { id: string }) {
  ...
  if (isLoading)
    return <div className="text-sm text-gray-700">Loading product…</div>;
  if (error || !data)
    ...
    <div className="space-y-2">
      <div>
        ...
        <div className="text-xs text-gray-600">
          SKU: {data.sku} • {data.type} • {data.status}
        </div>
      </div>
      {data.featuredImage && (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          src={String(data.featuredImage)}
          alt={data.name}
      ...
      {data.basePrice && (
        <div className="text-sm">
          <span className="text-gray-600">Base price:</span> {data.basePrice}
        </div>
      )}
      {data.description && (
        <div
          className="text-xs text-gray-800 prose prose-sm max-w-none"
