...
export class WooCommerceProductSyncService {
  ...
  private updateProgress(stage: SyncProgress['stage'], current: number, total: number, message: string) {
  ...
  async syncAll(): Promise<SyncResult> {
    ...
    try {
      ...
      this.updateProgress('products', 0, 100, 'Starting product synchronization...');
      ...
      this.updateProgress('variations', 0, 100, 'Starting variation synchronization...');
      ...
      this.updateProgress('complete', 100, 100, 'Synchronization completed successfully!');
  ...
  private async syncCategories(): Promise<{ created: number; updated: number; errors: string[] }> {
    ...
    try {
      ...
      while (hasMore) {
        try {
          const response = await this.client.get(`/products/categories?page=${page}&per_page=${perPage}&hide_empty=false`) as WooCommerceCategoryDetailed[];
          allCategories.push(...response);
          hasMore = response.length === perPage;
          page++;
          
          this.updateProgress('categories', allCategories.length, allCategories.length + (hasMore ? perPage : 0), `Fetched ${allCategories.length} categories from WooCommerce...`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
          if (errorMessage.includes('cannot list resources') || errorMessage.includes('401')) {
            result.errors.push(`Authentication failed - API keys may not have read permissions. Please check WooCommerce REST API settings.`);
          } else {
            result.errors.push(`Failed to fetch categories page ${page}: ${errorMessage}`);
          }
          break;
        }
      }
      ...
      for (const [index, category] of rootCategories.entries()) {
        this.updateProgress('categories', index + 1, rootCategories.length + childCategories.length, `Processing root category: ${category.name}`);
        
        try {
          const syncData = this.mapCategoryToSyncData(category);
          const existing = await this.findExistingCategory(category.id.toString());
          ...
          result.errors.push(`Failed to sync category ${category.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }

      // Sync child categories
      for (const [index, category] of childCategories.entries()) {
        this.updateProgress('categories', rootCategories.length + index + 1, rootCategories.length + childCategories.length, `Processing child category: ${category.name}`);
        ...
        try {
          ...
          if (category.parent > 0) {
            const parentCategory = await this.findExistingCategory(category.parent.toString());
            if (parentCategory) {
  ...
  private async syncProducts(): Promise<{ created: number; updated: number; errors: string[] }> {
    const result = { created: 0, updated: 0, errors: [] as string[] };
    ...
    try {
      ...
      while (hasMore) {
        try {
          const response = await this.client.get(`/products?page=${page}&per_page=${perPage}`) as WooCommerceProduct[];
          allProducts.push(...response);
          hasMore = response.length === perPage;
          page++;
          
          this.updateProgress('products', allProducts.length, allProducts.length + (hasMore ? perPage : 0), `Fetched ${allProducts.length} products from WooCommerce...`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
          if (errorMessage.includes('cannot list resources') || errorMessage.includes('401')) {
            result.errors.push(`Authentication failed - API keys may not have read permissions. Please check WooCommerce REST API settings.`);
          } else {
            result.errors.push(`Failed to fetch products page ${page}: ${errorMessage}`);
          }
          break;
        }
      }
      ...
      for (const [index, product] of allProducts.entries()) {
        ...
        try {
          ...
          if (!existing && syncData.sku) {
            ...
            this.log(`After findExistingProductBySku(${syncData.sku}): ${existing ? JSON.stringify({id: existing.id, sku: existing.sku}) : 'null'}`);
          }
          
          let productId: string;
          this.log(`Final existing value: ${existing ? JSON.stringify({id: existing.id}) : 'null'}`);
          console.log(`DECISION POINT: existing =`, existing);
          console.log(`existing truthy?`, !!existing);
          
          if (existing) {
            console.log(`Updating existing product: ${existing.id} (WC ID: ${product.id})`);
            this.log(`Product ${product.name} (WC ID: ${product.id}) found as existing with DB ID: ${existing.id} - UPDATING`);
            await this.updateProduct(existing.id, syncData);
            productId = existing.id;
            result.updated++;
          } else {
            console.log(`Creating new product: ${product.name} (WC ID: ${product.id})`);
            this.log(`Product ${product.name} (WC ID: ${product.id}) not found as existing - CREATING`);
          ...
          result.errors.push(`Failed to sync product ${product.name}: ${errorMessage}`);
        }
      }

    } catch (error) {
      result.errors.push(`Product sync failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }

    return result;
  }

  private async syncVariations(): Promise<{ created: number; updated: number; errors: string[] }> {
    const result = { created: 0, updated: 0, errors: [] as string[] };

    try {
      // Get all variable products from our database
      const variableProducts = await db.select()
        .from(products)
        .where(and(
          eq(products.shopId, this.shopId),
          eq(products.type, 'variable')
        ));

      for (const [productIndex, product] of variableProducts.entries()) {
        if (!product.wooCommerceId) continue;
        
        this.updateProgress('variations', productIndex + 1, variableProducts.length, `Processing variations for: ${product.name}`);
        
        try {
          // Fetch variations for this product
          const variations = await this.client.get(`/products/${product.wooCommerceId}/variations?per_page=100`) as WooCommerceProductVariation[];
          
          for (const variation of variations) {
            try {
              const syncData = this.mapVariationToSyncData(variation, product.id);
              const existing = await this.findExistingVariation(variation.id.toString());
              ...
              result.errors.push(`Failed to sync variation ${variation.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
          }
        } catch (error) {
          result.errors.push(`Failed to sync variations for product ${product.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }

    } catch (error) {
      result.errors.push(`Variation sync failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }

    return result;
  }

  private mapCategoryToSyncData(category: WooCommerceCategoryDetailed): CategorySyncData {
    return {
  ...
  private async mapProductToSyncData(product: WooCommerceProduct): Promise<ProductSyncData> {
    ...
    return {
      ...
      status: product.status === 'publish' ? 'published' : product.status as 'draft' | 'private',
  ...
  private mapVariationToSyncData(variation: WooCommerceProductVariation, productId: string): VariationSyncData {
    // Helper function to convert empty strings to undefined for numeric fields
    const parseNumeric = (value: string | number | null | undefined): string | undefined => {
      if (value === '' || value === null || value === undefined) return undefined;
      return value.toString();
  ...
  private async findExistingProduct(wooCommerceId: string) {
    console.log(`Looking for existing product with WooCommerce ID: ${wooCommerceId}`);
    console.log(`Shop ID: ${this.shopId}`);
    
    // First check what's actually in the products table
    const allProducts = await db.select().from(products).limit(10);
    console.log(`All products in DB (first 10):`, allProducts.map(p => ({id: p.id, wooCommerceId: p.wooCommerceId, name: p.name})));
    ...
    if (this.debugMode) {
      ...
      this.log(`DB DEBUG - Looking for WC ID: ${wooCommerceId}, Shop ID: ${this.shopId}`);
      if (allProducts.length > 0) {
        this.log(`DB DEBUG - Sample products: ${JSON.stringify(allProducts.slice(0, 3).map(p => ({id: p.id, wooCommerceId: p.wooCommerceId, shopId: p.shopId})))}`);
      }
    }
    ...
    if (this.debugMode) {
      this.log(`DB DEBUG - Query result for WC ID ${wooCommerceId}: ${found ? `Found ID ${found.id}` : 'Not found'}`);
    }
  ...
  private async findExistingProductBySku(sku: string) {
    ...
    const found = results[0] || null;
    console.log(`Found existing product by SKU:`, found ? `ID ${found.id}` : 'None');
    return found;
  }
  ...
  private async updateCategory(id: string, data: CategorySyncData): Promise<void> {
  ...
  private async createProduct(data: ProductSyncData): Promise<string> {
    try {
      ...
      const result = await db.insert(products).values(productValues).returning({ id: products.id });
      ...
      throw new Error(`Database insertion failed for product ${data.name} (SKU: ${data.sku}): ${error instanceof Error ? error.message : 'Unknown database error'}`);
    }
  }

  private async updateProduct(id: string, data: ProductSyncData): Promise<void> {
    try {
      ...
      if (existingProduct.length === 0) {
        throw new Error(`Product with ID ${id} not found in database for update`);
      }
      ...
      if (existingProduct[0].sku !== data.sku) {
        ...
        if (skuConflict.length > 0 && skuConflict[0].id !== id) {
          this.log(`SKU CONFLICT: Product ${id} trying to update to SKU "${data.sku}" but it's already used by product ${skuConflict[0].id}`);
          throw new Error(`SKU "${data.sku}" is already in use by another product`);
        }
      }

      this.log(`UPDATE DEBUG: About to update product ID ${id}, current SKU: ${existingProduct[0].sku}, new SKU: ${data.sku}`);
      ...
      if (this.debugMode) {
        this.log(`UPDATE ERROR for product ID ${id}: ${error instanceof Error ? error.message : String(error)}`);
      }
      throw error;
    }
  }
  ...
  private async createVariation(data: VariationSyncData): Promise<string> {
    try {
      ...
      if (parentProduct.length === 0) {
        throw new Error(`Parent product with ID ${data.productId} not found for variation ${data.wooCommerceId}`);
      }

      if (this.debugMode) {
        this.log(`VARIANT CREATE DEBUG: Creating variation ${data.wooCommerceId} for existing product ${data.productId} (SKU: ${parentProduct[0].sku})`);
      }
      ...
      if (this.debugMode) {
        this.log(`VARIANT CREATE DEBUG: Successfully created variation ${data.wooCommerceId} with ID ${result[0].id}`);
      }
      
      return result[0].id;
    } catch (error) {
      if (this.debugMode) {
        this.log(`VARIANT CREATE ERROR for WC ID ${data.wooCommerceId}, product ID ${data.productId}: ${error instanceof Error ? error.message : String(error)}`);
      }
      throw error;
    }
  }

  private async updateVariation(id: string, data: VariationSyncData): Promise<void> {
  ...
  private async syncProductCategories(productId: string, wooCategories: { id: number; name: string; slug: string }[]): Promise<void> {
    ...
    for (const wooCategory of wooCategories) {
      const category = await this.findExistingCategory(wooCategory.id.toString());
      if (category) {
...
export async function createSyncServiceForShop(shopId: string): Promise<WooCommerceProductSyncService> {
