...
export async function POST(request: NextRequest) {
  try {
    ...
    const deletedOrphanedVariants = await db
    ...
    console.log(
      `🔸 Cleaned up ${deletedOrphanedVariants.length} orphaned product variants`
    );

    // Clean up orphaned product-category associations
    const deletedOrphanedProductCategories = await db
      ...
      .where(
        notExists(
          db
            .select()
            .from(products)
            .where(eq(products.id, productCategories.productId))
        )
      )
      .returning({
    ...
    console.log(
      `🔸 Cleaned up ${deletedOrphanedProductCategories.length} orphaned product-category associations (missing products)`
    );

    // Clean up product-category associations with missing categories
    const deletedOrphanedProductCategories2 = await db
      .delete(productCategories)
      .where(
        notExists(
          db
            .select()
            .from(categories)
            .where(eq(categories.id, productCategories.categoryId))
        )
      )
      .returning({
        productId: productCategories.productId,
    ...
    console.log(
      `🔸 Cleaned up ${deletedOrphanedProductCategories2.length} orphaned product-category associations (missing categories)`
    );

    // Clean up orphaned product-brand associations
    const deletedOrphanedProductBrands = await db
      ...
      .where(
        notExists(
          db
            .select()
            .from(products)
            .where(eq(products.id, productBrands.productId))
        )
      )
      .returning({
    ...
    console.log(
      `🔸 Cleaned up ${deletedOrphanedProductBrands.length} orphaned product-brand associations (missing products)`
    );

    // Clean up product-brand associations with missing brands
    const deletedOrphanedProductBrands2 = await db
      .delete(productBrands)
      .where(
        notExists(
    ...
    console.log(
      `🔸 Cleaned up ${deletedOrphanedProductBrands2.length} orphaned product-brand associations (missing brands)`
    );

    const totalAffected =
      deletedOrphanedVariants.length +
    ...
    return NextResponse.json({
      ...
      details: {
        ...
        orphanedProductCategories:
          deletedOrphanedProductCategories.length +
          deletedOrphanedProductCategories2.length,
        orphanedProductBrands:
          deletedOrphanedProductBrands.length +
          deletedOrphanedProductBrands2.length,
      },
    });
