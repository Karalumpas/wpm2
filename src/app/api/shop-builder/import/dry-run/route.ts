...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!shopId || !config) return NextResponse.json({ error: 'targetShopId and config required' }, { status: 400 });

    const s = await db.select().from(shops).where(eq(shops.id, shopId)).limit(1);
    if (!s.length) return NextResponse.json({ error: 'Shop not found' }, { status: 404 });
    ...
    return NextResponse.json({ ok: true, checks, summary: { productCount, categoryCount, tagsCount } });
  } catch (e) {
